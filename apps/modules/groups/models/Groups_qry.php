...
class Groups_qry extends CI_Model{
    ...
    public function select_main_group($param = null) {
        $this->db->where('maingroupid',NULL, FALSE);
        if($param){
    ...
    public function json_dgview() {
        ...
        $aColumns = array('privilege','name', 'description', 'id' );
	$sIndexColumn = "id";
        if(!empty($_GET['iDisplayLength'])){
            $sLimit = " LIMIT " . $_GET['iDisplayLength'];
        }
        $sTable = " (SELECT id as privilege
                        , name
                        , description
                        , id
                        FROM
        ...
        foreach ( $rResult->result_array() as $aRow )
        {
                ...
                $row[0] = "<a style=\"margin-bottom: 0px;\" class=\"btn btn-info btn-xs \" href=\"".site_url('groupaccess/index/'.$aRow['id'])."\">Set Privilege</a>";
                $row[3] = "<a style=\"margin-bottom: 0px;\" class=\"btn btn-default btn-xs \" href=\"".site_url('groups/edit/'.$aRow['id'])."\">Edit</a>";
